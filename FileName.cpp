...
void save_data(int inventory[], int sale[], int price[], char product_name[][SIZE])
{
	...
	for (int i = 1; i <= MAX_PRODUCTS; i++)
	{
		if (product_name[i][0] != '\n')
		{
			fprintf(fp, "%d %s %d %d %d\n", i, product_name[i], inventory[i], sale[i], price[i]);
		}
	}
...
void load_data(int inventory[], int sale[], int price[], char product_name[][SIZE])
{
	...
	int id, temp_inv, temp_sale, temp_price;
	char temp_name[SIZE];

	while (fscanf_s(fp, "%d %s %d %d %d",
		&id,
		temp_name, (unsigned)SIZE,
		&temp_inv,
		&temp_sale,
		&temp_price) == 5)
	{
		...
		{
			strcpy_s(product_name[id], SIZE, temp_name);
			inventory[id] = temp_inv;
			sale[id] = temp_sale;
			price[id] = temp_price;
		}
	}
